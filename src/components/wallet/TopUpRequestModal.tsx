              ...
              {paystackMethodVisible && (
                ...
                >
                  ...
                  <div
                    className="flex h-9 w-9 items-center justify-center rounded-full transition-colors"
                    style={{
                      backgroundColor: mode === 'instant'
                        ? `color-mix(in srgb, var(--success) 20%, transparent)`
                        : 'var(--border-color)',
                    }}
                  >
                    <FaBolt className="h-4 w-4" style={{ color: mode === 'instant' ? 'var(--success)' : 'var(--text-muted)' }} />
                  </div>
                  <div className="text-center leading-tight">
                    <p className="font-semibold">Instant Pay</p>
                    <p className="mt-0.5 text-xs" style={{ color: mode === 'instant' ? 'var(--success)' : 'var(--text-muted)' }}>
                      Via Paystack
                    </p>
                  </div>
                </button>
